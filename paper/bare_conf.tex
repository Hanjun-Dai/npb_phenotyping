...
\subsection{CANDECOMP/PARAFAC (CP) Tensor Factorization}
...
\begin{align*}
	\theta^{(p, k)} \sim & Dir(\gamma_p), k = 1, 2, \ldots \\
	\theta^{(m, k)} \sim & Dir(\gamma_m), k = 1, 2, \ldots \\
	\theta^{(d, k)} \sim & Dir(\gamma_d), k = 1, 2, \ldots \\
\end{align*}
